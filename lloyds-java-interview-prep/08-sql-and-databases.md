...
**Real-time example.** A transaction history query filters by status and then a date
range: `WHERE status = 'PENDING' AND created_at BETWEEN ? AND ?`. The index should be
`(status, created_at)` in that order — the equality column first, the range column second.
Reversed, as `(created_at, status)`, it is far less effective, because the range scan
happens first and status cannot narrow it.
...
**Real-time example.** A dashboard count of pending transactions per currency needs only
`status` and `currency`. An index on `(status, currency)` covers it, so the database
answers from the index and never reads a table row. On a large transactions table that is
