...
This is where a bank screens hardest for someone at your level. At under two years, the
technical round is mostly fundamentals — because fundamentals are what predict whether you
will write reliable code in an unfamiliar codebase. Expect rapid-fire questions here.
...
> the only way in is through methods that validate. On a trading platform an account
> balance cannot simply be assigned — it has to go through a method that checks the
> transaction is valid and the account is in a state that permits it. If that field were
> public, any code anywhere could set it and skip every check, and you would never find
...
> "On the trading platform we integrated **multiple payment vendors**, and that is the
> natural interface. `PaymentVendor` defines the contract — initiate, verify status,
> refund — and each vendor implements it completely differently. There is no shared code
> to inherit, only a shared contract, so the service depends on the interface and Spring
> injects the right implementation. Adding a vendor does not change the calling code.
>
> An **abstract class** would be right where implementations genuinely share state and
> behaviour — a base transaction type holding the amount, currency, timestamp and a common
> `validate()` that all of them run. That is shared code, which an interface cannot give
> me."
...
**Real-time example.** In a payment reconciliation job: a `List` for the page of
transactions being processed, a `Set` of already-processed reference numbers so a retry
does not process the same payment twice, and a `Map` of status code to description loaded
once instead of looked up per row.
...
**Real-time example.** A trade history endpoint loads a page of transactions and iterates
them to build the response — read-heavy, index access, no insertion in the middle. That is
an `ArrayList`. I have not yet hit a case in real work where `LinkedList` was the right
answer.
...
> "Two places. Reference data — instrument codes, currency codes, status descriptions —
> loaded once into a `HashMap` keyed by code, so a loop over transactions looks each one up
> in O(1) instead of hitting the database per row. And for idempotency: a map of
> already-processed request or message IDs, checked before acting, so a duplicate or
> retried request becomes a no-op instead of a second payment."
...
newest-first is its natural order everywhere in the application. But a trade history
screen lets the user sort by amount, by status or by instrument — three `Comparator`s,
...
**Real-time example.** On Tradu the backend served a React frontend, so a consistent error
contract mattered a lot — the frontend team needed one shape to handle. Custom unchecked
exceptions such as `TransactionNotFoundException` and an invalid-state exception, both
extending a common `BusinessException`, mapped through `@RestControllerAdvice` to 404 and
409. Unchecked also matters for `@Transactional`, because a checked exception would
