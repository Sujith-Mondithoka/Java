...
You authenticate once; you authorise on every request. On a trading platform,
authentication proves who the user is; authorisation decides whether they are a **trader**
acting on their own account, a **compliance reviewer** who can approve or reject an
onboarding case, or an **admin**.
...
**Your version:**
> "On Tradu the backend served a React frontend, and the rule that mattered was that a
> user must never be able to reach another user's account or perform an action their role
> does not permit. I worked on enforcing that at the API layer with Spring Security rather
> than relying on the UI, because a UI check only hides the button — anyone can call the
> endpoint directly with Postman. Authorisation has to be server side."
