...
At under two years nobody will ask you to design Netflix. But expect one design question,
possibly framed as "how would you build X" or "how would you improve this", and expect them
to be watching **how you think** rather than whether you produce a perfect architecture.

At a bank, the things that earn marks are the ones most candidates skip: what happens when a
dependency fails, how you would know in production, and how it gets released safely.
...
## Design 2 🔴 — A user onboarding / verification workflow
*Closest to your actual domain. If they let you pick an example, pick this.*

**Clarify:** What are the verification steps, and are any of them external? Who reviews a
case that fails automated checks? Does it need an audit trail? Can a user resume a
half-finished application?

**Data model:**
```
onboarding_case
  id, user_id, status, current_step, created_at, updated_at

verification_check
  id, onboarding_case_id, check_type, vendor_ref, outcome, checked_at

review_decision
  id, onboarding_case_id, reviewer_id, decision, comments, decided_at
...
- **Idempotency on submit**, so a double-click does not create two cases — or, on the
  payments side, two debits from one intent. This is the single most important rule on a
  financial platform.
...
**The rules to state:**
- **Nouns, not verbs.** `/accounts/{id}/withdrawals` as a sub-resource, not
  `/doWithdrawal?id=`.
...
3. Draw the onboarding state machine and say why the audit log is append-only.
