...
per status: how many transactions are pending, settled and failed, and the total value of
each. Written as nested loops with counters and a map built by hand, that is about twenty
lines. One `groupingBy` with `summingDouble` replaces it, and the intent is visible in a
single read.
...
**Real-time example.** An order has a list of executions, or an onboarding case has a list
of verification checks. To get every check across a page of cases, `map` gives me a stream
of *lists* of checks; `flatMap` gives me a single stream of checks I can filter and count.
