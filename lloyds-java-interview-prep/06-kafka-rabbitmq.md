# 06 · Kafka and Messaging 🟠
**Time needed: 30 minutes — read for understanding, not for depth**

⚠️ **Calibrate this one honestly.** Your resume says **"Apache Kafka fundamentals"**, which
is the right word. Hold that line in the interview: know the model well, and say plainly that
you have not run it in production. See file 11, section 5.2.

**Why it is still worth 30 minutes:** it is on your resume so it may be asked, and more
importantly the *idea* behind it — moving slow work off the request thread — comes up in
design questions whether or not Kafka is mentioned. Understanding why you would publish an
event instead of making a synchronous call is a genuinely useful answer.

Read Q1 (Kafka vs RabbitMQ), Q3 (Kafka concepts) and Q4. Skip the rest if you are short of
time.
...
> request. Moving it onto a queue means the API returns immediately, and a mail server outage
> stops being an API failure — the messages wait in the queue until it recovers."

On a trading platform the same shape applies to confirmation emails, audit events and
downstream reporting after a transaction settles. The rule is: **if the user does not need
it to continue, it does not belong in the request.**
