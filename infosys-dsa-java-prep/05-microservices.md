...
give up), **Q3** (circuit breaker), **Q4** (saga) and **Q7** (how to answer the depth
question honestly).
...
## Q7. 🔴 "How much microservices experience do you have?"

Your resume lists microservices under skills, and your experience is building Spring Boot
services on a large enterprise banking platform. Be precise about which of those is true,
because the follow-up will find out anyway.

### The answer

> "I should be clear about the level. On the Service Online platform I built Spring Boot
> services and REST APIs that other parts of the platform and a React frontend consumed, and
> the platform used Kafka and RabbitMQ for messaging between components. So I have worked in
> a distributed system and across service boundaries.
>
> What I have not done is own the decomposition of a system into services — deciding the
> boundaries, standing up discovery and a gateway. I understand how those work and why they
> are there."

**Then show the understanding**, which is where you recover the ground:

> "The way I think about it is that microservices buy independent deployment, independent
> scaling and fault isolation, and you pay with network failure, no distributed transaction,
> and much harder debugging. For a small system a well-structured monolith is usually the
> better engineering decision. The split earns its cost when teams start blocking each other
> or one part needs to scale differently."

**Saying the costs unprompted is what separates you** from candidates reciting the benefits.

### Follow-ups to be ready for
- *"How would you split a monolith?"* → By **business capability**, not technical layer.
  Mention the **strangler fig** pattern — proxy in front, move one capability at a time.
- *"How do services communicate?"* → Synchronous REST when you need the answer to continue;
  messaging when you are telling the system something happened. **You have a real example** —
  the audit and notification paths on your platform did not need to block the main request.
- *"What happens when a downstream service is slow?"* → Timeout always, then a circuit
  breaker and a fallback. See Q3.
