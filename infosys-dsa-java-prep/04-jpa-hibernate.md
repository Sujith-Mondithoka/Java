...
**This is the highest-value technical file for a Spring Boot backend interview.** You built
Spring Data JPA / PostgreSQL backends for two applications, so JPA questions are guaranteed
and they will go deeper than the annotations.

N+1 is the classic question here. **Q3 has a template to fill with your own example** — and
an honest fallback if you have not personally fixed one. Do not borrow a number from
anywhere; a made-up metric collapses on the first follow-up.
...
### 🔴 Your version of the answer — decide this before Saturday

**If you did fix one**, use this frame with something real:

> "[Where it showed up.] [How I found it — SQL logging, and I saw the same query repeating
> once per row.] [What I changed — join fetch, `@EntityGraph`, batch fetching or a DTO
> projection.] [The effect — a number if you measured one, otherwise 'it went from one query
> per row to a single query'.]"

**If you have not**, answer on the mechanism. This scores well and cannot be broken:

> "I have not had to fix one myself, but I know what it is and how I would find it. If an
> endpoint were slow I would turn on SQL logging and count the queries — the same query
> repeating once per row is N+1. I would fix it with a join fetch or an `@EntityGraph`, or a
> DTO projection for a read-only screen, then re-check the query count."

### What you genuinely can claim, and should
From your own work at Standard Bank:

- **The Camunda workflow on Business Card**, which cut card processing time by **60%** —
  that is your headline number and it is a *workflow* story, not a query-tuning one. Do not
  blur the two.
- **The AOP audit system** writing **50,000+ transactions a day** to PostgreSQL. That is a
  volume story and a clean-architecture story. See file 11.
- **JasperReports** generating **1,000+ monthly reports**, and **FileNet** for document
  storage.
- **Liquibase** for versioned schema migrations and **Lombok** to cut persistence
  boilerplate — both worth mentioning here because they are JPA-adjacent and few candidates
  bring them up.
...
**Real-time example.** Branch codes, product codes and transaction-status descriptions
...
2. Give your N+1 answer in under 60 seconds — real example or the honest mechanism version.
